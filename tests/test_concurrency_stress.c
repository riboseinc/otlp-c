...
static void *worker(void *arg)
{
	...
	for (i = 0; i < SPANS_PER_THREAD; i++) {
		...
		otlp_span_set_attribute_int(span, "thread.id",
...
int main(void)
{
	...
	echo_server_join(&srv, 5 * 1000 * 1000);
	...
	if (total_failures > 0) {
		fprintf(stderr, "[stress] FAIL: %d emit failures\n",
			total_failures);
		otlp_exporter_free(g_exp);
		otlp_tracer_free(g_tracer);
		return 1;
