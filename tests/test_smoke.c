...
int main(void)
{
	...
	printf("major=%d minor=%d patch=%d\n",
	       OTLP_C_VERSION_MAJOR, OTLP_C_VERSION_MINOR, OTLP_C_VERSION_PATCH);
	...
	assert(span == NULL);  /* stub returns NULL */

	otlp_exporter_opts_t opts = { 0 };
	otlp_exporter_t *exp = otlp_exporter_create(&opts);

	assert(exp == NULL);  /* stub returns NULL */

	printf("[smoke] PASS — library loads and links, stubs behave\n");
	return 0;
}
