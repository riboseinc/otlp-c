...
prop_traceparent_roundtrip(uint64_t seed)
{
	...
	if (otlp_traceparent_format(span, sampled, buf1, sizeof(buf1), &len1)
	    != OTLP_OK)
		goto out;
	if (otlp_traceparent_parse(buf1, trace_id, span_id, &flags)
	    != OTLP_OK)
		goto out;
...
prop_traceparent_reject_zero(uint64_t seed)
{
	...
	if (otlp_traceparent_parse(
		"00-00000000000000000000000000000000-"
		"0000000000000000-00",
		trace_id, span_id, &flags) == OTLP_OK)
		return 0;
	return 1;
}
...
prop_traceparent_reject_malformed(uint64_t seed)
{
	...
	if (otlp_traceparent_parse(
		"0g-0af7651916cd43dd8448eb211c80319c-"
		"b7ad6b7169203331-01",
		trace_id, span_id, &flags) == OTLP_OK)
		return 0;
	if (otlp_traceparent_parse(
		"00_0af7651916cd43dd8448eb211c80319c-"
		"b7ad6b7169203331-01",
		trace_id, span_id, &flags) == OTLP_OK)
		return 0;
	if (otlp_traceparent_parse("short",
		trace_id, span_id, &flags) == OTLP_OK)
		return 0;
	return 1;
}
