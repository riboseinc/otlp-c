...
prop_flush_metric_variants(uint64_t seed)
{
...

	{
		...
		for (size_t i = 0; i < sizeof(types) / sizeof(types[0]); i++)
		{
			...
			m = otlp_metric_create(types[i],
				"m",
				"",
				"",
				(types[i] == OTLP_METRIC_HISTOGRAM) ? bounds
								    : NULL,
...
prop_flush_exp_histogram_with_buckets(uint64_t seed)
{
	...
	m = otlp_metric_create(OTLP_METRIC_EXP_HISTOGRAM,
		"latency",
		"ns",
...
main(void)
{
	...
	failures += property_run(prop_flush_metric_null_transport,
		"prop_flush_metric_null_transport",
		10,
		1);
	failures += property_run(prop_flush_log_null_transport,
		"prop_flush_log_null_transport",
		10,
		1);
	failures += property_run(
		prop_flush_metric_variants, "prop_flush_metric_variants", 5, 1);
	failures += property_run(prop_flush_exp_histogram_with_buckets,
		"prop_flush_exp_histogram_with_buckets",
		5,
		1);
