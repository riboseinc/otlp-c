...
prop_extract_rejects_crlf_baggage(uint64_t seed)
{
	...
	if (!ctx_in.has_context)
		goto out_fail;
	snprintf(ctx_in.baggage, sizeof(ctx_in.baggage), "%s", "key=value");
	otlp_context_inject(ctx_in, carrier_set, &carrier);
...
prop_format_raw_matches_format(uint64_t seed)
{
	...
	st = otlp_traceparent_format(
		span, true, buf_span, sizeof(buf_span), NULL);
	if (st != OTLP_OK)
		goto out;
	st = otlp_traceparent_format_raw(
		trace_id, span_id, true, buf_raw, sizeof(buf_raw), NULL);
	if (st != OTLP_OK)
		goto out;
