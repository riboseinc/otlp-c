...
mini_serve_thread(void *arg)
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		snprintf(buf,
			...
			"\r\n"
			"hi",
			s->send_connection_close ? "Connection: close\r\n"
						 : "");
		(void) resp;
		...
		otlp_atomic_fetch_add_int(
			&s->requests_served, 1, OTLP_MEMORY_ORDER_RELEASE);
...
prop_keepalive_disabled_on_explicit_close(uint64_t seed)
{
	...
	if (otlp_http_parse_url(url_str, &url) != OTLP_OK)
		goto out;
	if (otlp_http_request_start(&req,
		    &url,
		    "test",
		    NULL,
		    0,
...
prop_keepalive_reuse_roundtrip(uint64_t seed)
{
	...
	if (otlp_http_parse_url(url_str, &url) != OTLP_OK)
		goto out;
	if (otlp_http_request_start(&req1,
		    &url,
		    "test",
		    NULL,
		    0,
	...
	if (!sock)
		goto out;
	if (otlp_http_request_start_with_socket(&req2,
		    &url,
		    "test",
		    NULL,
		    0,
	...
	ok = (len2 == 2 && memcmp(body2, "hi", 2) == 0 &&
		otlp_atomic_load_int(
			&srv.requests_served, OTLP_MEMORY_ORDER_RELAXED) == 2);
out:
	if (req1)
