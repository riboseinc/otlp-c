...
producer_main(void *arg)
{
	...
	for (i = 0; i < ITEMS_PER_PRODUCER; i++)
	{
		...
		uintptr_t item = (uintptr_t)(uint64_t)(i + 1) << 16 |
		...
		if (st != OTLP_OK)
		{
			...
			atomic_store_explicit(
				pa->done_flag, 1, memory_order_release);
			return (void *) 1;
		}
	}
	return NULL;
...
prop_mpsc_no_loss_no_dup(uint64_t seed)
{
	...
	for (i = 0; i < N_PRODUCERS; i++)
	{
		...
		if (pthread_create(
			    &threads[i], NULL, producer_main, &args[i]) != 0)
			goto out;
	}

	seen = calloc((size_t) N_PRODUCERS * ITEMS_PER_PRODUCER, sizeof(*seen));
	if (!seen)
		goto out;
...
main(void)
{
	...
	failures += property_run(
		prop_mpsc_no_loss_no_dup, "prop_mpsc_no_loss_no_dup", 5, 1);
