...
test_span_struct_size(void)
{
	 ...
	 * arrays. */
	check_true(otlp_span_struct_size() <= 512);
...
test_span_attribute_upsert(void)
{
	...
	check_true(span != NULL);
	check_true(otlp_span_set_attribute_int(span, "k", 1) == OTLP_OK);
	check_true(
		otlp_span_set_attribute_string(span, "other", "x") == OTLP_OK);
	check_true(otlp_span_set_attribute_string(span, "k", "replaced") ==
		OTLP_OK);
	attrs = otlp_span_get_attrs(span, &n);
	check_true(n == 2);
	check_true(strcmp(attrs[0].key, "k") == 0);
...
test_span_attribute_array_kvlist(void)
{
	...
	check_true(span != NULL);
	check_true(otlp_span_set_attribute_array(span, "tags", items, 3) ==
		OTLP_OK);
	check_true(
		otlp_span_set_attribute_kvlist(span, "map", kvs, 2) == OTLP_OK);
	attrs = otlp_span_get_attrs(span, &n);
	check_true(n == 2);
	check_true(attrs[0].type == OTLP_ATTR_ARRAY);
	check_true(attrs[0].v.array_val->n == 3);
	check_true(attrs[0].v.array_val->items[0].v.int64_val == 7);
	check_true(strcmp(attrs[0].v.array_val->items[1].v.string_val,
			   "seven") == 0);
	check_true(attrs[0].v.array_val->items[2].v.bool_val == true);
	check_true(attrs[1].type == OTLP_ATTR_KVLIST);
	check_true(attrs[1].v.kvlist_val->n == 2);
	check_true(strcmp(attrs[1].v.kvlist_val->entries[0].key, "a") == 0);
	check_true(attrs[1].v.kvlist_val->entries[1].value.v.double_val == 2.5);

	/* Upsert over a composite: the old tree is released, the
	 * scalar takes its place. */
	check_true(otlp_span_set_attribute_int(span, "tags", 1) == OTLP_OK);
	attrs = otlp_span_get_attrs(span, &n);
	check_true(n == 2);
	...
	check_true(n == 2);
	check_true(attrs[1].type == OTLP_ATTR_KVLIST);
	check_true(attrs[1].v.kvlist_val != NULL);
...
test_event_typed_attributes(void)
{
	...
	check_true(span != NULL);
	check_true(otlp_span_add_event(span, "e", 1) == OTLP_OK);
	check_true(otlp_span_set_event_attribute_int(span, "attempt", 3) ==
		OTLP_OK);
	check_true(otlp_span_set_event_attribute_double(span, "ratio", 0.5) ==
		OTLP_OK);
	check_true(otlp_span_set_event_attribute_bool(span, "final", true) ==
		OTLP_OK);
	check_true(otlp_span_set_event_attribute_bytes(
			   span, "ctx", payload, 2) == OTLP_OK);
	ev = otlp_span_get_events(span, &n);
	check_true(n == 1 && ev[0].attrs.n == 4);
	check_true(ev[0].attrs.items[0].type == OTLP_ATTR_INT64);
	check_true(ev[0].attrs.items[0].v.int64_val == 3);
	check_true(ev[0].attrs.items[1].type == OTLP_ATTR_DOUBLE);
	check_true(ev[0].attrs.items[1].v.double_val == 0.5);
	check_true(ev[0].attrs.items[2].type == OTLP_ATTR_BOOL);
	check_true(ev[0].attrs.items[2].v.bool_val == true);
	check_true(ev[0].attrs.items[3].type == OTLP_ATTR_BYTES);
	check_true(ev[0].attrs.items[3].v.bytes_val.len == 2);
	check_true(ev[0].attrs.items[3].v.bytes_val.data[1] == 0xff);
	otlp_span_free(span);
	return 0;
}
...
test_link_typed_attributes(void)
{
	...
	check_true(span != NULL);
	check_true(otlp_span_add_link(span, tid, sid) == OTLP_OK);
	check_true(
		otlp_span_set_link_attribute_int(span, "weight", 9) == OTLP_OK);
	check_true(otlp_span_set_link_attribute_double(span, "score", 1.25) ==
		OTLP_OK);
	check_true(otlp_span_set_link_attribute_bool(span, "cached", false) ==
		OTLP_OK);
	check_true(otlp_span_set_link_attribute_bytes(
			   span, "tag", payload, 2) == OTLP_OK);
	lk = otlp_span_get_links(span, &n);
	check_true(n == 1 && lk[0].attrs.n == 4);
	check_true(lk[0].attrs.items[0].type == OTLP_ATTR_INT64);
	check_true(lk[0].attrs.items[0].v.int64_val == 9);
	check_true(lk[0].attrs.items[1].type == OTLP_ATTR_DOUBLE);
	check_true(lk[0].attrs.items[1].v.double_val == 1.25);
	check_true(lk[0].attrs.items[2].type == OTLP_ATTR_BOOL);
	check_true(lk[0].attrs.items[2].v.bool_val == false);
	check_true(lk[0].attrs.items[3].type == OTLP_ATTR_BYTES);
	check_true(lk[0].attrs.items[3].v.bytes_val.len == 2);
	check_true(lk[0].attrs.items[3].v.bytes_val.data[0] == 0x0a);
	otlp_span_free(span);
	return 0;
}
...
test_event_link_overflow(void)
{
	...
	for (i = 0; i < 64; i++)
		check_true(otlp_span_add_event(span, "e", 1) == OTLP_OK);
	check_true(otlp_span_add_event(span, "e", 1) == OTLP_ERR_OVERFLOW);
	for (i = 0; i < 64; i++)
		check_true(otlp_span_add_link(span, tid, sid) == OTLP_OK);
	check_true(otlp_span_add_link(span, tid, sid) == OTLP_ERR_OVERFLOW);
	otlp_span_free(span);
	return 0;
}
...
test_event_link_attr_before_add(void)
{
	...
	check_true(span != NULL);
	check_true(otlp_span_set_event_attribute_string(span, "k", "v") ==
		OTLP_ERR_INVALID_ARGUMENT);
	check_true(otlp_span_set_link_attribute_string(span, "k", "v") ==
		OTLP_ERR_INVALID_ARGUMENT);
	check_true(otlp_span_set_event_attribute_int(span, "k", 1) ==
		OTLP_ERR_INVALID_ARGUMENT);
	check_true(otlp_span_set_link_attribute_bytes(span, "k", NULL, 0) ==
		OTLP_ERR_INVALID_ARGUMENT);
	otlp_span_free(span);
	return 0;
}
