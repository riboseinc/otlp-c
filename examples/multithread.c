...
typedef struct { otlp_exporter_t *exp; otlp_tracer_t *tracer; int thread_id; int n; } worker_arg_t;
...
static otlp_thread_t thread_create_worker(worker_arg_t *a) {
    return CreateThread(NULL, 0, worker_thread, a, 0, NULL);
}
static void thread_join(otlp_thread_t t) { WaitForSingleObject(t, INFINITE); CloseHandle(t); }
#else
#include <pthread.h>
...
static void thread_join(otlp_thread_t t) { pthread_join(t, NULL); }
#endif
...
main(void)
{
    ...
    tracer = otlp_tracer_create(
        "multithread-demo", "demo", otlp_version());
    if (!tracer) {
    ...
    if (stats.emitted != (uint64_t) total_expected) {
        fprintf(stderr, "[multithread] FAIL: emitted %llu != %d\n",
                (unsigned long long) stats.emitted, total_expected);
        return 1;
    }
