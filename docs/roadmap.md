...
| 1.1.3 | FreeBSD promoted to a gating CI check — removing continue-on-error exposed 3 real portability bugs the jobs API had been masking (Apple-only Availability.h, feature-hidden INADDR_LOOPBACK, undeclared memmem), all fixed via tests/test_portable.h; CLAUDE.md key-files gains exporter_sync; site /changelog | #170 |
